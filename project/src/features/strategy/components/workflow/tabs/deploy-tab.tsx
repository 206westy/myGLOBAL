  ...
  if (!data || data.length === 0) {
    return (
      <div className="p-4">
        <EmptyQueue
          tabLabel="Deploy"
          hint="Journey B auto-matching and rollout tracking — activated in P1 alongside Where-used analysis."
        />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3 p-4">
      <p className="text-sm text-muted-foreground">
        {data.length} item(s) waiting in Deploy phase. Decision UI will be added in P1.
      </p>
      {data.map((r) => (
